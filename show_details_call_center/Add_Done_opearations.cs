...
{
    internal class Add_done_operation
    {
        ...
        string data;
        string table_type;
        public Add_done_operation(qafla qafla, string data, string Accountname, string location,string table_type)
        {
            this._qafla = qafla;
            this.data = data;
        ...
        public void insert_done_operations()
        {
            ...
            try
            {
                string cm5 = $"DELETE FROM delay_operations WHERE [كود المريض]=N'{_qafla.code}'";
            ...
            try
            {
                string cm5 = $"DELETE FROM Accepted_operation WHERE [كود المريض]=N'{_qafla.code}'";
                SqlCommand cmd5 = new SqlCommand(cm5, conn);
                cmd5.ExecuteNonQuery();
            }
            catch { }
            ...
            try
             {
                string cm2 = $"DELETE FROM cancel_operations WHERE [كود المريض]=N'{_qafla.code}'";
                SqlCommand cmd1 = new SqlCommand(cm2, conn);
            ...
            try
            {
                string cm2 = $"DELETE FROM other_operations WHERE [كود المريض]=N'{_qafla.code}'";
                SqlCommand cmd1 = new SqlCommand(cm2, conn);
                cmd1.ExecuteNonQuery();
            }
            catch { }
            ...
            conn.Close();
            conn.Open();
            string sql = $"UPDATE {AccountName} SET تم=N'اجريت' WHERE [كود المريض]=N'{_qafla.code}'";
